...
namespace BeryllUtils
{
    ...
    public:
        ...
        {
            BR_ASSERT((glm::length(v1) > 0.9999f && glm::length(v1) < 1.0001f &&
                       glm::length(v2) > 0.9999f && glm::length(v2) < 1.0001f), "%s", "Vectors not normalized.");
        ...
        {
            BR_ASSERT((glm::length(start) > 0.9999f && glm::length(start) < 1.0001f &&
                       glm::length(dest) > 0.9999f && glm::length(dest) < 1.0001f), "%s", "Vectors not normalized.");
        ...
        static VectorSide getVectorSide(const glm::vec3& vectorBase, const glm::vec3& vectorToCheck)
        {
            BR_ASSERT((glm::length(vectorBase) > 0.9999f && glm::length(vectorBase) < 1.0001f &&
                       glm::length(vectorToCheck) > 0.9999f && glm::length(vectorToCheck) < 1.0001f), "%s", "Vectors not normalized.");

            if(getIsVectorsParallelInSameDir(vectorBase, vectorToCheck))
                return VectorSide::PARALLEL_SAME_DIR;
            else if(getIsVectorsParallelInOppositeDir(vectorBase, vectorToCheck))
                return VectorSide::PARALLEL_OPPOSITE_DIR;

            float dotProduct = glm::dot(vectorBase, BeryllConstants::worldUp);
            if(dotProduct == 1.0f || dotProduct == -1.0f)
                return VectorSide::ERROR; // v1 and BeryllConstants::worldUp are parallel, we can not find right vector.

            glm::vec3 rightVector = glm::normalize(glm::cross(vectorBase, BeryllConstants::worldUp));

            if(glm::acos(glm::dot(rightVector, vectorToCheck)) < glm::half_pi<float>())
